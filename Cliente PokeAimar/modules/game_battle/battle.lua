...
local function onTabPress()
    ...
    for k, v in pairs(battleButtonsByCreaturesList) do
        if (v.creature:isAttackable() and localPlayer:hasSight(v.creature:getPosition()) and
                v.creature:canBeSeen()) then
            list[#list + 1] = v
        end
    end
    localPlayer = nil
    ...
    local target, next
    for _, battleButton in pairs(list) do
        if (next) then
            target = battleButton
            break
    ...
    if (target) then
        g_game.attack(target.creature)
        lastTarget = target.creature
    end
end
...
function init()
  ...
  sortTypeBox:addOption(tr('Name'), 'name')
  sortTypeBox:addOption(tr('Distance'), 'distance')
  sortTypeBox:addOption(tr('Age'), 'age')
  sortTypeBox:addOption(tr('Health'), 'health')
  ...
  g_keyboard.bindKeyPress('Tab', onTabPress)
...
function removeAllCreatures()
  creatureAgeList = {}
  for _, battleButton in pairs(battleButtonsByCreaturesList) do
    removeCreature(battleButton.creature, true)
  end
end

function removeCreature(creature, forced)
