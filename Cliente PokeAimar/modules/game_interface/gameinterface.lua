...
function init()
  ...
  g_ui.importStyle('styles/leaving')
...
function onLevelChange(localPlayer, value, percent)
  loginLevel = g_game.getLocalPlayer():getLevel()
end
...
function onSendAndExit()
  if (exitWindow and exitWindow:getChildById('text') and exitWindow:getChildById('text') ~= '') then
    g_game.reportBug("== LEAVING MESSAGE ==\n" .. exitWindow:getChildById('text'):getText())
  end
  g_game.safeLogout()
  forceExit()
end
...
function doShowWyalseWindow() -- Why You Are Leaving So Early
  if (not exitWindow) then
    if (loginLevel <= 5 and g_game.getLocalPlayer():getLevel() <= 15) then -- Starter that doesnt get level 16 at least
      exitWindow = g_ui.createWidget('LeavingWindow', modules.game_interface.getRootPanel())
      exitWindow:getChildById('text'):focus()
      return true
    end
  end
  return false
...
function tryExit()
  if --[[doShowWyalseWindow() or ]]exitWindow then
    return true
  end
...
local function doGreetNpc(creatureThing, localPosition)
  if (Position.distance(creatureThing:getPosition(), localPosition) <= 4) then
    modules.game_console.doFocusNPCTab()
    modules.game_console.sendMessage(tr('Hi'))
  else
    ...
    for k, v in pairs(Directions) do
      pos = Position.getPositionByDirection(pos, v)
      if (localPlayer:autoWalk(pos)) then
        walking = true
        break
      end
    end
    ...
    if (walking) then
      local stopEventCount = 0
      periodicalEvent(
        function()
          print(Position.distance(creatureThing:getPosition(), localPlayer:getPosition()))
          if (Position.distance(creatureThing:getPosition(), localPlayer:getPosition()) < 4) then
            g_game.talkChannel(MessageModes.Say, 0 , 'Hi')
          end
          stopEventCount = stopEventCount + 1
        end,
        function()
          return stopEventCount <= 50
        end,
        1000)
    end]]
end
...
function createThingMenu(menuPosition, lookThing, useThing, creatureThing)
  ...
  if creatureThing then
    ...
    if creatureThing:isLocalPlayer() then
      ...
      menu:addOption(tr('Teleport'), function() g_game.talkChannel(MessageModes.Say, 0 , '/tp') end)
      menu:addOption(tr('Toggle Autoloot'), function() g_game.talkChannel(MessageModes.Say, 0, '/autoloot') end)
      menu:addOption(tr('Cancel Duel'), function()
          local item = localPlayer:getInventoryItem(InventorySlotBack)
          if item:isItem() then
            g_game.useWith(item, creatureThing)
          end
        end)
      ...
      if (creatureThing:isLocalPlayerSummon()) then
        menu:addOption(tr('Evolve'), function() g_game.useInventoryItem(11595) end)
        menu:addOption(tr('Set Outfit'), function() g_game.talkChannel(MessageModes.Say, 0, '/addon') end)
      end
      ...
      if creatureThing:isPlayer() then
        ...
        menu:addOption(tr('Invite/Accept to Duel'), function()
            local item = localPlayer:getInventoryItem(InventorySlotBack)
            if item:isItem() then
              g_game.useWith(item, creatureThing)
            end
      ...
      if (creatureThing:isNpc()) then
        menu:addOption(tr('Talk'), function() doGreetNpc(creatureThing, localPosition) end)
      end
    end
