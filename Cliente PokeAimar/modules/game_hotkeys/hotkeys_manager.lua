...
function init()
  ...
  useOnPokemon = hotkeysWindow:getChildById('useOnPokemon')
  ...
  useRadioGroup:addWidget(useOnPokemon)
...
function loadDefautComboKeys()
  if not defaultComboKeys then
    for i=1,12 do
      addKeyCombo('F' .. i, {autoSend = true, itemId = nil, useType = HOTKEY_MANAGER_USE, value = "#s m" .. i})
    end
    for i=1,4 do
...
function doKeyCombo(keyCombo)
  ...
  elseif hotKey.useType == HOTKEY_MANAGER_USEONPOKEMON then
    if g_game.getProtocolVersion() < 780 or hotKey.subType then
      local item = g_game.findPlayerItem(hotKey.itemId, hotKey.subType or -1)
      if item then
        local pos = g_game.getLocalPlayer():getPosition()
        if (pos) then
          for i, c in ipairs(g_map.getSpectators(pos, false)) do
            if (c:isLocalPlayerSummon()) then
              g_game.useWith(item, c)
            end
          end
        end
      end
      ...
      if (pos) then
        for i, c in ipairs(g_map.getSpectators(pos, false)) do
          if (c:isLocalPlayerSummon()) then
            g_game.useInventoryItemWith(hotKey.itemId, c)
          end
        end
      end
    end
...
function updateHotkeyLabel(hotkeyLabel)
  ...
  elseif hotkeyLabel.useType == HOTKEY_MANAGER_USEONPOKEMON then
  ...
  if currentHotkeyLabel then
    ...
    if currentHotkeyLabel.itemId ~= nil then
      ...
      if currentItemPreview:getItem():isMultiUse() then
        ...
        elseif currentHotkeyLabel.useType == HOTKEY_MANAGER_USEONPOKEMON then
          useRadioGroup:selectWidget(useOnPokemon)
...
function onChangeUseType(useTypeWidget)
  ...
  elseif useTypeWidget == useOnPokemon then
    currentHotkeyLabel.useType = HOTKEY_MANAGER_USEONPOKEMON
