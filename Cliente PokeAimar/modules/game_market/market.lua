...
buyStatsTable = nil
sellStatsTable = nil
ownBuyOfferTable = nil
ownSellOfferTable = nil
...
local ownOfferTableHeader = {
    {['text'] = 'Item', ['width'] = 135},
    {['text'] = 'Amount', ['width'] = 95},
    {['text'] = 'Total Price', ['width'] = 125},
    {['text'] = 'Piece Price', ['width'] = 115},
    {['text'] = 'Ends at', ['width'] = 155}
}
...
local function isItemValid(item, category, searchFilter)
  ...
  if searchFilter then
    return marketData.name:lower():find(searchFilter:lower())
  end
  return true
end
...
local function clearOffers()
  ...
  ownMarketOffers[MarketAction.Buy] = {}
  ownMarketOffers[MarketAction.Sell] = {}
  buyOfferTable:clearData()
  sellOfferTable:clearData()
  ownBuyOfferTable:clearData()
  ownSellOfferTable:clearData()
end
...
local function addOffer(offer, type, own)
  if not offer then
    return false
  end
  local id = offer:getId()
  local playerOrItem = own and getItemNameByClientId(offer:getItem():getId()) or offer:getPlayer()
  ...
  if type == MarketAction.Buy then
    local data = {
      {['text'] = playerOrItem, ['width'] = own and 135 or 100},
      {['text'] = amount, ['width'] = own and 95 or 60},
      {['text'] = price*amount, ['width'] = own and 125 or 90},
      {['text'] = price, ['width'] = own and 115 or 80},
      {['text'] = string.gsub(os.date('%c', timestamp), " ", "  "), ['width'] = own and 155 or 120}
    }
    if (own) then
        ownBuyOfferTable:addRow(data, id)
    else
    ...
    if (own) then
        ownSellOfferTable:addRow(data, id)
    else
...
local function mergeOffer(offer, own)
  if not offer then
    return false
  end
  local id = offer:getId()
  ...
  local offerTable = own and ownMarketOffers or marketOffers

  if type == MarketAction.Buy then
    for i = 1, #offerTable[MarketAction.Buy] do
      local o = offerTable[MarketAction.Buy][i]
      -- replace existing offer
      if o:isEqual(id) then
          offerTable[MarketAction.Buy][i] = offer
        replaced = true
      end
    end
    if not replaced then
      table.insert(offerTable[MarketAction.Buy], offer)
    end
  else
    for i = 1, #offerTable[MarketAction.Sell] do
      local o = offerTable[MarketAction.Sell][i]
      -- replace existing offer
      if o:isEqual(id) then
          offerTable[MarketAction.Sell][i] = offer
        replaced = true
      end
    end
    if not replaced then
      table.insert(offerTable[MarketAction.Sell], offer)
    end
  end
  return true
end
...
local function updateOwnOffers(offers)
    if not ownBuyOfferTable or not ownSellOfferTable then
        return
    end

    balanceLabel:setColor('#bbbbbb')
    ownSelectedOffer[MarketAction.Buy] = nil
    ownSelectedOffer[MarketAction.Sell] = nil

    --[[if (#offers == 1) then
        for k, offer in pairs(ownCancelingOffers) do
            if (offers[1]:getCounter() == offer:getCounter()) then
                local t = offer:getType() == MarketAction.Buy and ownBuyOfferTable or ownSellOfferTable
    ...
    cancelSellButton:setEnabled(false)
    cancelBuyButton:setEnabled(false)

    for _, offer in pairs(offers) do
        mergeOffer(offer, true)
    end

    for i = 1, #offers do
        addOffer(offers[i], offers[i]:getType(), true)
    end
end
...
local function updateBalance(balance)
  ...
  balanceLabel:setText('Balance: '..balance..' dollar' .. (balance > 1 and 's' or ''))
...
local function openAmountWindow(callback, type, actionText)
  ...
  scrollbar:setText(tostring(selectedOffer[type]:getPrice())..' '..tr('dollars'))
  scrollbar:setMaximum(max)
  scrollbar:setMinimum(1)
  scrollbar:setValue(1)

  scrollbar.onValueChange = function(widget, value)
    widget:setText(tostring(value*selectedOffer[type]:getPrice())..' '..tr('dollars'))
...
local function onSelectOwnBuyOffer(table, selectedRow, previousSelectedRow)
    updateBalance()
    for _, offer in pairs(ownMarketOffers[MarketAction.Buy]) do
        if offer:isEqual(selectedRow.ref) then
            ownSelectedOffer[MarketAction.Buy] = offer
            cancelBuyButton:setEnabled(true)
        end
    end
end

local function onSelectOwnSellOffer(table, selectedRow, previousSelectedRow)
    updateBalance()
    for _, offer in pairs(ownMarketOffers[MarketAction.Sell]) do
        if offer:isEqual(selectedRow.ref) then
            ownSelectedOffer[MarketAction.Sell] = offer
            cancelSellButton:setEnabled(true)
        end
    end
end
    ...
    if newItem then
      ...
      if not table.empty(marketData) then
        if marketData.category == category or category == MarketCategory.All then
          ...
          marketData.name = getItemNameByClientId(t:getId())
...
local function initInterface()
  ...
  --overviewPanel = g_ui.loadUI('ui/marketoffers/overview') TODO
  ...
  local myOffersTabClick = myOffersTab.onClick -- Backup original method
  myOffersTab.onClick = function()
      myOffersTabClick(myOffersTab) -- Call original method (must parse the tab as paremeter)
      clearOffers()
      MarketProtocol.sendMarketBrowse(MarketRequest.MyOffers) -- Request player's current offers
  end
  ...
  --offerHistoryPanel = g_ui.loadUI('ui/myoffers/offerhistory') TODO
  ...
  cancelBuyButton = myOffersPanel:recursiveGetChildById('cancelBuyButton')
  cancelBuyButton.onClick = function()
      local messageBox
      local defaultCallback = function() messageBox:ok() end
      messageBox = UIMessageBox.display(tr("Cancel Buy Offert"), tr("You really want to cancel this buy offert?"),
          {
              {text='Yes', callback=function()
                  local offer = ownSelectedOffer[MarketAction.Buy]
                  if (not offer) then
                      return
                  end
                  --ownCancelingOffers[#ownCancelingOffers + 1] = offer
                  MarketProtocol.sendMarketCancelOffer(offer:getTimeStamp(), offer:getCounter())
                  defaultCallback()
              end},
              {text='No', callback=defaultCallback}
          }, defaultCallback, defaultCallback)
  end

  cancelSellButton = myOffersPanel:recursiveGetChildById('cancelSellButton')
  cancelSellButton.onClick = function()
      local messageBox
      local defaultCallback = function() messageBox:ok() end
      messageBox = UIMessageBox.display(tr("Cancel Sell Offert"), tr("You really want to cancel this sell offert?"),
          {
              {text='Yes', callback=function()
                  local offer = ownSelectedOffer[MarketAction.Sell]
                  if (not offer) then
                      return
                  end
                  --ownCancelingOffers[#ownCancelingOffers + 1] = offer
  ...
  ownBuyOfferTable = myOffersPanel:recursiveGetChildById('buyingTable')
  ownSellOfferTable = myOffersPanel:recursiveGetChildById('sellingTable')
  buyOfferTable.onSelectionChange = onSelectBuyOffer
  sellOfferTable.onSelectionChange = onSelectSellOffer
  ownBuyOfferTable.onSelectionChange = onSelectOwnBuyOffer
  ownSellOfferTable.onSelectionChange = onSelectOwnSellOffer
end
...
function Market.onMarketEnter(depotItems, offers, balance, vocation)
  ...
  if ownBuyOfferTable and not ownBuyOfferTable:hasHeader() then
      ownBuyOfferTable:addHeaderRow(ownOfferTableHeader)
  end
  if ownSellOfferTable and not ownSellOfferTable:hasHeader() then
      ownSellOfferTable:addHeaderRow(ownOfferTableHeader)
  end
...
function Market.onMarketBrowse(offers, own)
  if (own) then
      updateOwnOffers(offers)
  else
    updateOffers(offers)
  end
end
