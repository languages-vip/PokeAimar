...
local function doUpdateStatusCount()
    local onlineCount = 0
    ...
    for i=1, childrenCount do
        ...
        if (child.vipState == VipState.Online) then
    ...
    vipWindow:setText(tr('VIP List') .. " [" .. onlineCount .. "/" .. onlineCount + offlineCount .. "]")
end
...
function onAddVip(id, name, state, description, iconId, notify)
  ...
  if not g_game.getFeature(GameAdditionalVipInfo) then
    ...
    if tmpVipInfo then
      if tmpVipInfo.iconId then
        label:setImageClip(torect((tmpVipInfo.iconId * 16) .. ' 0 16 16'))
    ...
    label:setImageClip(torect((iconId * 16) .. ' 0 16 16'))
    label.iconId = iconId
  ...
  if (not refreshing) then -- This is done directly into the refresh method
    doUpdateStatusCount()
  end
end
